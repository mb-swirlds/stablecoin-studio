...
export class HederaWalletConnectTransactionAdapter extends HederaTransactionAdapter {
  ...
  public async register(hWCSettings: HWCSettings): Promise<InitializationData> {
    ...
    if (!hWCSettings) throw new Error('hedera wallet conenct settings not set');
  ...
  public subscribe(): void {
    ...
    this.dAppConnector.walletConnectClient?.on('session_delete', async () => {
      await this.stop();
    });
  }
  ...
  async sign(message: string | Transaction): Promise<string> {
    ...
    try {
      ...
      if (!signedTxn) {
        throw new Error('❌ No signed transaction returned from WalletConnect');
      }
      ...
      if (firstEntry.done) {
        throw new Error('No signatures found in first SignaturePairMap');
      }
